...
export default function PageSkills() {
  ...
  return (
    <div ref={ref} style={{ width:"100vw", height:"100vh", background:"#f0f7e6", display:"flex", overflow:"hidden" }}>
      ...
      {/* LEFT */}
      <div style={{ width:300, flexShrink:0, display:"flex", flexDirection:"column", justifyContent:"center", padding:"64px 24px 24px 80px", borderRight:"1px solid rgba(0,0,0,0.06)" }}>
        <motion.div initial={{ opacity:0, x:-30 }} animate={inView?{opacity:1,x:0}:{}} transition={{ duration:0.7 }}>
          ...
          <h2 style={{ fontFamily:"var(--serif)", fontSize:"clamp(24px,2.8vw,36px)", lineHeight:1.0, letterSpacing:"-0.02em", color:"var(--text)", marginBottom:16 }}>
            What<br/>I<br/>Know
          </h2>
          <p style={{ fontSize:10, color:"var(--text-light)", lineHeight:1.75, marginBottom:20 }}>8 core domains · 70+ skills across languages, backend, frontend, cloud, data, security, architecture, and tools.</p>
          <div style={{ display:"flex", flexDirection:"column" as const, gap:6 }}>
            {["Expert","Advanced","Proficient"].map(tag=>(
              <div key={tag} style={{ display:"flex", alignItems:"center", gap:6, fontSize:9, color:"var(--muted)" }}>
      ...
      <div style={{ flex:1, height:"100vh", overflowY:"hidden", padding:"36px 28px 8px 0", display:"flex", flexDirection:"column" as const, justifyContent:"flex-start" }}>
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.7 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:8, paddingLeft:1 }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap" as const, gap:6, marginBottom:10 }}>
            {certifications.map((cert,i)=>(
              <motion.div key={cert.name}
                ...
                style={{ display:"flex", alignItems:"center", gap:8, padding:"5px 10px", background:cert.bg, border:`1px solid ${cert.border}`, borderRadius:100, transition:"all 0.25s", cursor:"default" }}
                ...
                <span style={{ fontSize:12 }}>{cert.icon}</span>
                <div>
                  <div style={{ fontSize:10, fontWeight:700, color:"var(--text)", lineHeight:1.2 }}>{cert.name}</div>
                  <div style={{ fontSize:9, color:cert.color, letterSpacing:"0.05em" }}>{cert.issuer} · {cert.level}</div>
                </div>
                <span style={{ fontSize:8, color:cert.color, fontWeight:700, background:cert.bg, padding:"1px 5px", borderRadius:100, border:`1px solid ${cert.border}`, marginLeft:4, whiteSpace:"nowrap" as const }}>{cert.year}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
